	...
	freeboard
			.loadWidgetPlugin({
				...
				"external_scripts" : [
						...
						"js/bimarian/plugins/plugin_highcharts_theme.js" ],
				"fill_size" : true,
...
var gTitles = currentSettings.graphTitles;
var srsArr = gTitles.split(',');
console.log(srsArr);
			for (i = 1; i <= MAX_NUM_SERIES; i++) {
		...
		yAxis: {
            ...
            maxPadding: 0.2,
            title: {
                text: currentSettings.yaxistitle
            },
			labels: {
				style: {
					color: '#333333',
...
labels: {
	...
	style: {
		...
		self.onCalculatedValueChanged = function(settingName, newValue) {
			...
			var data = [];
			_.each(newValue, function(ranking) {
			var sss = new Date(parseInt(ranking[0])); 
			var ss = sss.getUTCHours() + ':' + sss.getUTCMinutes() + ':' + sss.getUTCSeconds();
			data.push([ss,parseInt(ranking[1])]);
			});
			series.setData(data);
			}
