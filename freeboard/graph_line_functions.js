...
//for dynamic channels		
   var SupervisorFile="tenants.cfg";
	...
	//alert(hbase_rest_api_url);
	 ...
	 if(channelval=='VRL') {
	$.getJSON('bimarian_final.json', function(data) {
		...
		device.splice(0, 4,
			{
				...
				"type":"JSON",
				"settings":{
					"url":hbase_rest_api_url+"/getTransformedRecords?recordsCount=10&tenantId="+channelval+"&deviceId="+deviceval,
					"use_thingproxy":true,
					"refresh":5,
					"method":"GET"
				}
				...
				if (resp.status == 'RUNNING') {				
					var intevalVar = setInterval(function() {
						...
						//console.log("Internal result : " + internalResponse.status);								
							if (internalResponse.status == 'SUCCEEDED' || internalResponse.status == 'KILLED' || internalResponse.status == 'FAILED') {
								$('#loadingmessage').hide();
											...
											device.splice(0, 4,
												{
													...
													"type":"JSON",
													"settings":{
														"url":hbase_rest_api_url+"/getAnalyzedRecord?tenantId=VRL&deviceId=batch-analytics",
														"use_thingproxy":true,
														"refresh":5,
														"method":"GET"
													}
